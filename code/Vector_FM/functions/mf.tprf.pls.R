...
step1_select_Phi <- function(X_lf, Z,
  ...
  colnames(Phi_hat_sel) <- colnames(Z)
  
  for (i in 1:N) {
    df_i  <- data.frame(x = X_lf[, i], Z)
    
    # ---- SENZA intercetta ----
    mod_i <- lm(x ~ . - 1, data = df_i)
    ...
    # slope: ora sono TUTTI i coefficienti (non c'è intercetta)
    alpha_hat_i <- coef(mod_i)
    
    if (Lproxy == 1) {
      # t-test robusto: ora è la PRIMA riga (non la seconda)
      ...
      # Wald/F congiunto su tutte le proxy
...
# ==============================================================================
# PROXIES' ESTIMATION IN THE MF-3PRF: 
#===============================================================================
...
choose_UMIDAS_lag <- function(X_lf, X_hf, y_q,
  ...
  # --------------------------------------------------------
  # STEP 1 senza intercetta (solo se X_lf e Z sono centrate)
  XtX_1 <- t(Z) %*% Z           # Lproxy x Lproxy
  XtY_1 <- t(Z) %*% X_lf        # Lproxy x N
  
  Beta  <- solve(XtX_1, XtY_1)  # Lproxy x N
  Phi_hat_full <- t(Beta)       # N x Lproxy
