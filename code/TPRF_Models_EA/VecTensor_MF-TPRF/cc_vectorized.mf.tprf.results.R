# ==============================================================================
# Vectorized-from-Tensor MF-TPRF - Results Script
# ------------------------------------------------------------------------------
# This script:
...
path_results <- file.path(path_main, "TPRF_Models_EA/VecTensor_MF-TPRF/results/outputs_vec")
...
if (is.null(dates_m) || is.null(dates_q) || is.null(y_true_q)) {
  stop("The full-sample RDS must contain dates_m, dates_q, and y_q, directly or via inputs.")
}
...
plot_nowcast <- ggplot() +
  ...
  labs(
    title = paste0("Vectorized-from-Tensor MF-TPRF Monthly Nowcast - ", country),
    x = "Date",
    y = "GDP Growth"
  ) +
...
latex_insample <- latex_table_periods(
  mat     = rmsfe_insample,
  caption = paste0("Vectorized-from-Tensor MF-TPRF RMSFE by period -- ", country, " (", Size, ", sel = ", sel, ")"),
  label   = paste0("tab:vectensor_mf_tprf_", country, "_", Size, "_", sel)
)
...
if (is.null(dates_q_rt) || is.null(y_q_rt)) {
  stop("The pseudo real-time RDS must contain dates_q and y_q, directly or via inputs.")
}
...
plot_rt <- ggplot() +
  ...
  labs(
    title    = paste0("Vectorized-from-Tensor MF-TPRF Rolling Real-Time Nowcasts - ", country),
...
latex_rt <- latex_table_periods(
  mat     = rmsfe_rt,
  caption = paste0("Vectorized-from-Tensor MF-TPRF Rolling RMSFE by period -- ", country, " (", Size, ", sel = ", sel, ")"),
  label   = paste0("tab:vectensor_mf_tprf_rt_", country, "_", Size, "_", sel)
)
...
file_summary <- build_result_filename(
  ...
  L_midas          = L_midas,
  p_ar             = p_ar,
  r1               = if (!is.null(hyper_fit$r_hat)) hyper_fit$r_hat else NA,
  r2               = NA,
...
saveRDS(summary_vectensor_out, file_summary)
