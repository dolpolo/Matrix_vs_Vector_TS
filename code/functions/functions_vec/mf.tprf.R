...
build_autoproxy_3prf <- function(X_lf, y_q, Lproxy) {
  ...
  for (L in 1:(Lproxy - 1)) {
    ...
    # -------------------------
    # Step 2: fattore mirato con intercetta time-varying
    # F_t = X_lf * Phi * (Phi' Phi)^{-1}
    # -------------------------
    Phi_tilde <- cbind(1, Phi)                           # N x (k+1)
    XtX_2      <- t(Phi_tilde) %*% Phi_tilde             # (k+1) x (k+1)
    XtY_2      <- t(Phi_tilde) %*% t(X_lf)               # (k+1) x T
    M_2        <- solve(XtX_2, XtY_2)                    # (k+1) x T
    F_tildehat <- t(M_2)                                 # T x (k+1)
    
    # F_hat = fattori; prima colonna = intercetta time-varying
    F_hat      <- F_tildehat[, -1, drop = FALSE]
...
choose_UMIDAS_lag <- function(X_lf, X_hf, y_q,
  ...
  XtY_2 <- t(Phi_tilde) %*% t(X_hf)                  # (1+Lproxy) x T_m
...
MF_TPRF <- function(X_lf, X_hf, y_q,
  ...
  if (L_midas > 0) {
    ...
    beta_mat <- matrix(0, nrow = 0, ncol = 3 * K)
  }
  # --------------------------------------------------------
