...
choose_UMIDAS_grid_tensor_MF <- function(
  ...
  if (length(rmax) != 2) stop("rmax must be c(rmax1,rmax2).")
  rmax1 <- as.integer(rmax[1])
  rmax2 <- as.integer(rmax[2])
  if (rmax1 < 1 || rmax2 < 1) stop("Invalid rmax.")
  ...
  for (p_AR in 0:p_AR_max) {
    for (L in 1:Lmax) {
      ...
      X_MID <- NULL
      for (ell in 0:(L - 1)) {
        idx <- (start_tau_global - ell):(T_q - ell)
        X_MID <- cbind(
          X_MID,
          F1[idx, , drop = FALSE],
          F2[idx, , drop = FALSE],
...
#     rmax : c(rmax1, rmax2)       (max number of row/column factors for Ahn ER)
#   Output:
#     list(R, C, Z_used, Mlist, obj_path, r_selected, S_R0, S_C0, er_row, er_col)
# ==============================================================================
...
mf_tprf_pass1_ils <- function(
  ...
  rmax1 <- as.integer(rmax[1])
  rmax2 <- as.integer(rmax[2])
  if (rmax1 < 1 || rmax2 < 1) stop("rmax entries must be >= 1.")
  rmax1 <- min(rmax1, p1 - 1L)
  rmax2 <- min(rmax2, p2 - 1L)
  if (rmax1 < 1 || rmax2 < 1) stop("Need p1>=2 and p2>=2 to apply eigenvalue ratio.")
  ...
  # ---- build S_R0 and S_C0, then select r1 and r2 via Ahn-Horenstein ER
  ...
  er_row <- select_num_factors_ER(S_R0, Kmax = rmax1)
  er_col <- select_num_factors_ER(S_C0, Kmax = rmax2)
  
  r1 <- as.integer(er_row$r)
  r2 <- as.integer(er_col$r)
...
matrix_mf_tprf_autoproxy <- function(
  ...
  if (length(rmax) != 2) stop("rmax must be c(rmax1,rmax2).")
  rmax1 <- as.integer(rmax[1]); rmax2 <- as.integer(rmax[2])
  if (rmax1 < 1 || rmax2 < 1) stop("Invalid rmax.")
