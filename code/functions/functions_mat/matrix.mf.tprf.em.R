...
init_CL_ER <- function(X_std, W, kmax, max_iter = 20, do_plot = TRUE) {
  # 1. Stima dei ranghi (r1, r2) con ER su Cen & Lam
  r <- mfm.f(
    Y_std   = X_std,
    W       = W,
    kmax    = kmax,
  ...
  # 2. Imputazione finale con i ranghi stimati
  imp <- mfm.cl(
    Y = X_std,
    W = W,
    r = r
  ...
  list(
    X_init = imp$Y_imputed,  # [T x d1 x d2], senza NA
    r      = r,
    R_hat  = imp$R,
    C_hat  = imp$C,
    F_hat  = imp$f
